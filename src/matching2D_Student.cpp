                      ...
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType, bool crossCheck)
{
    ...
    { // nearest neighbor (best match)
        ...
        if (matcherType.compare("MAT_BF") == 0)
            std::cout << " Match Brutal Force : crossCheck = " << std::boolalpha << crossCheck << std::endl;
    }
    ...
    { // k nearest neighbors (k=2)
        ...
        t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
        std::cout << " (KNN) with n=" << matchesVec.size() << " matches in " << 1000 * t / 1.0 << " ms" << std::endl;

        // TODO : filter matches using descriptor distance ratio test
        double minDescDistRatio = 0.8;
        for (auto itr = matchesVec.begin(); itr != matchesVec.end(); ++itr)
        {
        ...
        // std::cout << " Num of keypoints removed = " << matchesVec.size() - matches.size() << std::endl;
        // std::cout << " Num of keypoints removed " 
        // << (matchesVec.size() - matches.size()) / (double(matchesVec.size())) * 100 << "%" << std::endl;
    }
}
    ...
    for (size_t r = 0; r < dst_norm.rows; ++r)
    {
        for (size_t c = 0; c < dst_norm.cols; ++c)
        {
            ...
            { // only store points above a threshold
                ...
                cv::KeyPoint newKeyPoint;
                newKeyPoint.pt = cv::Point2f(c, r); // row = y, col = x
                ...
                for (auto itr = keypoints.begin(); itr != keypoints.end(); ++itr)
                {
                    ...
                    double kptOverlap = cv::KeyPoint::overlap(newKeyPoint, *itr);
                    if (kptOverlap > maxOverlap)
                    {
                        bOverlap = true;
                        if (newKeyPoint.response > itr->response)
                        {                       // if overlap is >t AND response is higher for new kpt
                            *itr = newKeyPoint; // replace old key point with new one
...
void detKeypointsModern(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, std::string detectorType, bool bVis)
{
    ...
    if (bVis)
    {
        ...
        string windowName = "";
        cv::drawKeypoints(img, keypoints, visImage, cv::Scalar::all(-1), cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
        windowName = detectorType + std::string("Detector Results");
        cv::namedWindow(windowName, 6);
        imshow(windowName, visImage);
        cv::waitKey(0);
    }
